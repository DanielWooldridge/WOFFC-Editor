...
void ToolMain::UpdateInput(MSG * msg)
{
	...
	if (m_toolInputCommands.mouse_LB_Down)
	{
		...
		// Convert to client coordinates (relative to render window)
		ScreenToClient(m_toolHandle, &pt);

		// Check if click was inside the DirectX render area
	...
	if (m_keyArray['0'])
	{
		m_toolInputCommands.arc_Cam_Enabled = true;
	}
	else m_toolInputCommands.arc_Cam_Enabled = false;

	//WASD
}

void ToolMain::UpdateObjectInDisplayList()
{
...
void ToolMain::ToggleArcball()
{
	// Flip boolean
	m_d3dRenderer.camera.m_isArcballOn = !m_d3dRenderer.camera.m_isArcballOn;

	if (m_d3dRenderer.camera.m_isArcballOn && m_selectedObject >= 0 && m_selectedObject < m_sceneGraph.size())
	{
		const SceneObject& obj = m_sceneGraph[m_selectedObject];
		DirectX::SimpleMath::Vector3 target(obj.posX, obj.posY, obj.posZ);
		m_d3dRenderer.camera.arcballTarget = target;
	}
}

Camera* ToolMain::GetCamera()
{
	return &m_d3dRenderer.camera;
}
