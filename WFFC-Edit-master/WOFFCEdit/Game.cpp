...
void Game::Update(DX::StepTimer const& timer)
{
	...
	camera.Update(
		...
		m_InputCommands.rotLeft,
		m_InputCommands.rotRight,
		m_InputCommands.mouse_RB_Down,
		mouse,
		m_InputCommands.arc_Cam_Enabled
	);

	// Set updated view matrix
...
void Game::Render()
{
	...
	WCHAR Buffer[256];
	GetCursorPos(&mouse);
	std::wstring var2 = L"mouse X: " + std::to_wstring(mouse.x) + L"mouse y: " + std::to_wstring(mouse.y);
	m_font->DrawString(m_sprites.get(), var2.c_str(), XMFLOAT2(100, 10), Colors::Yellow);
	m_sprites->End();
...
void Game::UpdateDisplayList(int selectedID, std::vector<SceneObject>* sceneGraph)
{
	// Update m_displayList with the latest SceneObject info
	// Replace or update data for the selected object

	if (selectedID >= 0 && selectedID < m_displayList.size())
	{
		const SceneObject& updatedObject = sceneGraph->at(selectedID);

		m_displayList[selectedID].m_position = DirectX::SimpleMath::Vector3(
			updatedObject.posX,
			updatedObject.posY,
			updatedObject.posZ
		);

		m_displayList[selectedID].m_orientation = DirectX::SimpleMath::Vector3(
			updatedObject.rotX,
			updatedObject.rotY,
			updatedObject.rotZ
		);

		m_displayList[selectedID].m_scale = DirectX::SimpleMath::Vector3(
			updatedObject.scaX,
			updatedObject.scaY,
			updatedObject.scaZ
		);
